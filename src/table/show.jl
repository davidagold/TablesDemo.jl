using AbstractTables:   getmaxwidths,
                        rightbound,
                        show_tbl_header,
                        show_tbl_rows

function AbstractTables.show_tbl(io, tbl::Table, rowlabel, displaysummary, limit, offset)
    if ncol(tbl) > 0
        displaysummary && println(io, summary(tbl))
        show_predicate_aliases(io, tbl)
...
function show_predicate_aliases(io, tbl)
